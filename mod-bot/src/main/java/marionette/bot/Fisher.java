...
import marionette.bot.mixin.FishingHookAccessor;
...
final class Fisher {
    ...
    synchronized String begin() {
        ...
        if (slot < 0) slot = MarionetteBot.takeFromBackpack(p, "fishing_rod");
    ...
    synchronized void tick() {
        ...
        if (!p.getMainHandItem().is(Items.FISHING_ROD)) {
            ...
            if (another < 0) another = MarionetteBot.takeFromBackpack(p, "fishing_rod");
            if (another >= 0) {
        ...
        hookTicks++;
        if (((FishingHookAccessor) hook).marionetteBiting()) {
            mc.gameMode.useItem(p, InteractionHand.MAIN_HAND);
            fishCaught++;
            dry = 0;
            launched = false;
