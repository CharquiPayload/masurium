...
final class Follower {
    ...
    synchronized void tick() {
        ...
        if (me != null && (me.isSleeping() || MarionetteBot.goingToSleep())) return;
        ...
        Route.Point here = MarionetteBot.whereAmI(world, me);
        ...
        boolean boss = MarionetteBot.isMyBoss(target);
        ...
        Route.Options op = new Route.Options(
                MarionetteBot.safeFall(me.getHealth()), nodes, build, true)
    ...
    private boolean handleObstruction(Minecraft mc, LocalPlayer me) {
        ...
        for (Route.Point m : goals) {
            Route.Result res = Route.search(world, here, m, new Route.Options(
                    MarionetteBot.safeFall(me.getHealth()), 800, false));
    ...
    synchronized String state() {
        if (!following) {
            return String.format("{\"following\":false,\"outcome\":\"%s\"}",
                    marionette.common.Request.escape(outcome));
        }
        return String.format("{\"following\":true,\"to\":\"%s\"}",
                marionette.common.Request.escape(target));
    }
