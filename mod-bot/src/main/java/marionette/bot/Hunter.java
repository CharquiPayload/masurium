...
final class Hunter {
    ...
    synchronized void tick() {
        ...
        Route.Result r = Route.search(world, here,
                ...
                new Route.Options(MarionetteBot.safeFall(p.getHealth()), 8_000,
        ...
        if (d > 30 && MarionetteBot.approachTo(p, world, here,
    ...
    private void searchPreys(Minecraft mc, LocalPlayer p) {
        ...
        if (!MarionetteBot.approachTo(p, world, here, ox, oz, walker)) {
    ...
    private static int shearsSlot(LocalPlayer p, boolean climb) {
        ...
        for (int i = 0; i < inv.getContainerSize(); i++) {
            ...
            return MarionetteBot.takeFromBackpack(p, "shears");
        }
        return -1;
    }
    ...
    private boolean pickUpDropped(Minecraft mc, LocalPlayer p) {
        ...
        Route.Result r = Route.search(world, here, goal, new Route.Options(
                MarionetteBot.safeFall(p.getHealth()), 8_000, false));
        if (!r.hasRoute()
