 ...
 * config/marionette-server.txt} at startup. EVERY per-world state file hangs from this
...
final class ServerIdentity {
    ...
    static synchronized String key() {
        ...
        key = "unknown";
        try {
            Path mark = Path.of("config", "marionette-server.txt");
