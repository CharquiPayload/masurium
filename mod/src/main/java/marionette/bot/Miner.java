...
final class Miner {
    ...
    synchronized void tick() {
        ...
        if (mc.level.getBlockState(target).isAir()) {
            ...
            if (Places.forget(target)) {
                marionette.common.Logbook.note("places", String.format(
    ...
    private static int withToolOrWarn(Minecraft mc, BlockPos where, LocalPlayer p,
        ...
        if (inBag >= 0) {
            ...
            int ascent = MarionetteBot.takeFromBackpack(p, id);
            if (ascent >= 0) {
                marionette.common.Logbook.note("dig", String.format(
    ...
    static int spare(LocalPlayer p, BlockState state) {
        ...
        String inBag = toolInBackpack(p, state);
        if (inBag != null) {
            int ascent = MarionetteBot.takeFromBackpack(p, inBag);
