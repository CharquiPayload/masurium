...
/**
 * A bot on a server without Masurium leaves it, and closes its game.
 *
 ...
 *
 * <p>The sign is the {@code /masurium} command: the server half registers it for
 * everyone, and a server sends each client its command tree when it joins. No such
 * command a few seconds after joining means no Masurium there.
 */
public final class ServerCheck {

    /** What the server half registers, open to every player. */
    public static final String COMMAND = "masurium";
    ...
    public static void onTick(ClientTickEvent.Post event) {
        ...
        switch (judge(has, ticks)) {
            ...
            case LEAVE -> {
                ...
                LogUtils.getLogger().error("[masurium-bot] {} has no Masurium on its side (no /{} command): "
