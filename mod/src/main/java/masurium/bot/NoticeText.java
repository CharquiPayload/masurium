...
public final class NoticeText {
    ...
    static final String URL =
            "https://github.com/CharquiPayload/masurium/blob/main/docs/setup.md";
    ...
    static List<Notice> noticesFor(boolean isBot, boolean blankName, String badName,
        ...
        if (badName != null) {
            ...
            out.add(new Notice("bad-name",
                    "-Dmasurium.name=" + badName + " is not a name a bot can use",
        ...
        if (blankName) {
            ...
            out.add(new Notice("blank-name",
                    "-Dmasurium.name is empty, so this instance is NOT a bot",
                    "Give it a name: -Dmasurium.name=Alice",
                    Severity.ERROR, false));
            return out;
        }
        ...
        if (!isBot) {
            out.add(new Notice("not-a-bot",
                    "Masurium is installed but this instance is not a bot",
        ...
        out.add(new Notice("ready",
                "Masurium: this instance is the bot " + name,
        ...
        if (!hasServer) {
            out.add(new Notice("no-server",
                    "-Dmasurium.server is missing, so this bot will not join on its own",
                    "Add -Dmasurium.server=192.168.1.10:25565, or pick a server below",
                    Severity.ERROR, true));
        }
